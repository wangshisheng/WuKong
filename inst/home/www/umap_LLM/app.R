...
###########
ui <- dashboardPage(
  title = "UMAP with LLM Assistance",
  dashboardHeader(title = "UMAP with LLM Assistance", titleWidth = 350),
  dashboardSidebar(
    sidebarMenu(
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        umaptextvalue1<<-grnames1
        umapclassname<<-grnames
        umap_leibie_num<<-grnum1
        umap_fenzu_num<<-grnum2
        umapdata<<-peaksdataout()
        if(ncol(umapdata)==1){
          ggplot() + 
          ...
          library(ggpubr)
          library(ellipse)
          #set.seed(123) random_state uwot::umap2 umap::umap
          #datareadt<-uwot::umap2(t(umapdata),n_neighbors=4,n_components=2,min_dist=0.001,
          #                       fast_sgd=F,seed = 123)
          #umapdf<-as.data.frame(datareadt)#$layout
          datareadt<-umap::umap(t(umapdata),n_neighbors=4,n_components=2,random_state = 1234)
          umapdf<-as.data.frame(datareadt$layout)
          umapdf$Groups<-umapclassname
          colnames(umapdf)[1:2]<-c("p1","p2")
          centroids <- aggregate(cbind(p1,p2)~Groups,umapdf,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(umapdf$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(umapdf[umapdf$Groups==t,1:2]),
                                                               centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                               level=0.95),stringsAsFactors=FALSE)))
          umapdf$labelnames<-rownames(umapdf)
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_umap.plot",usertimenum,".pdf",sep="")},
        content = function(file){
...
ggplot(data=umapdf,(aes(x=p1,y=p2,colour = Groups)))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_umap.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_umap.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
