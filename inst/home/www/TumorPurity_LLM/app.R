...
###########
ui <- dashboardPage(
  title = "Estimate tumor purity with LLM Assistance",
  dashboardHeader(title = "Estimate tumor purity with LLM Assistance", titleWidth = 460),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              colorPicker(
                inputId = "pointcolx",
                label = h5("3.1. Select a color for samples:"),
                choices = list(
                  "npg"=pal_npg("nrc")(10),
                  "white"="white",
                  "grey"="grey",
              ...
              numericInput("labelsizex",h5("3.2. Point size:"),3),
              numericInput("heightx",h5("3.3. Figure Height:"),550),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        tumorpdata<<-peaksdataout()
        if(ncol(tumorpdata)==1){
          datareadx<-data.frame(Description="No Results here. Please upload your dataset or use the example data.")
        }else{
          library(tidyestimate)
          filtered <- filter_common_genes(tumorpdata,id = "hgnc_symbol",tidy = FALSE,tell_missing = TRUE, 
                                          find_alias = TRUE)
          scores <- estimate_score(filtered,is_affymetrix = TRUE)
          #Extract the score results as a data frame
          datareadx <- scores
          #datareadx<-plot_purity(scores,is_affymetrix = TRUE)
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_tumorpurity.table",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(tumorpdata)==1){
          ...
          pointsizex<<-isolate(input$labelsizex)
          library(tidyestimate)
          filtered <- filter_common_genes(tumorpdata,id = "hgnc_symbol",tidy = FALSE,tell_missing = TRUE, 
                                          find_alias = TRUE)
          scores <- estimate_score(filtered,is_affymetrix = TRUE)
          ...
          purity_line <- dplyr::tibble(estimate = seq(from = global_min,to = global_max, by = 10), purity = cos(0.6049872018 + 0.0001467884 * .data$estimate))
          ggplot(tidyestimate::purity_data_affy, aes(x = .data$estimate)) + 
            geom_point(aes(y = .data$purity_observed), alpha = 0.2,shape = 1) + 
            geom_line(aes(y = .data$ci_95_low), linetype = "dashed", alpha = 0.3) + 
            geom_line(aes(y = .data$ci_95_high), linetype = "dashed", alpha = 0.3) + 
            geom_line(data = purity_line, aes(y = .data$purity), alpha = 0.4) + 
            geom_point(data = scores, aes(y = .data$purity), color = pointcolx, size = pointsizex, shape = 1) + 
            ...
            labs(x = "ESTIMATE score", y = "Tumor purity")
        }
      ...
      ClusterCorNetplotout<-reactive({
        ...
        if(ncol(tumorpdata)==1){
          ...
          pointsizex<<-isolate(input$labelsizex)
          library(tidyestimate)
          filtered <- filter_common_genes(tumorpdata,id = "hgnc_symbol",tidy = FALSE,tell_missing = TRUE, find_alias = TRUE)
          scores <- estimate_score(filtered,is_affymetrix = TRUE)
          scores_nona <- dplyr::filter(scores, !is.na(.data$purity))
          global_min <- min(scores_nona$estimate, tidyestimate::purity_data_affy$estimate, na.rm = T)
          global_max <- max(scores_nona$estimate, tidyestimate::purity_data_affy$estimate, na.rm = T)
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_tumorpurity.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please show me the whole R codes. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(tidyestimate::purity_data_affy, aes(x = .data$estimate)) + 
          ...
          geom_line(aes(y = .data$ci_95_high), linetype = "dashed", alpha = 0.3) + 
          geom_line(data = purity_line, aes(y = .data$purity), alpha = 0.4) + 
          geom_point(data = scores, aes(y = .data$purity), color = "red", size = 3, shape = 1) + 
          ggrepel::geom_text_repel(data = scores, aes(y = .data$purity, label = .data$sample)) + 
          coord_cartesian(ylim = c(0, 1)) + 
          theme(panel.background = element_rect(fill = NA, color = "black"), panel.grid = element_blank()) + 
          labs(x = "ESTIMATE score", y = "Tumor purity")
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_tumorpurity.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_tumorpurity.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
