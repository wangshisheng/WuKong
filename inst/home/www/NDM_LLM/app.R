...
###########
ui <- dashboardPage(
  title = "Network Degree Matrix (NDM) with LLM Assistance",
  dashboardHeader(title = "Network Degree Matrix (NDM) with LLM Assistance", titleWidth = 495),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              radioButtons(
                "resultsout",
                label = NULL,
                choices = list("4.1. Tables"=1),#"4.1. Visualization" = 1,
                selected = 1,
                inline = TRUE
              ),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
          ...
          for(i in 1:nrow(NDMdata)){
            ...
            while(k <= ncol(NDMdata)){
              ...
              if(s >= h){
                ...
                upper[i,s2[k:k+s]]<-NDMdata[i,s2[min(ncol(NDMdata),k+s+h)]]
                lower[i,s2[k:k+s]]<-NDMdata[i,s2[max(n0*(n0>h)+1,k-h)]]
              }
              k <- k+s+1
            }
          }
          n2<-ncol(NDMdata)
          B <-ndm<-matrix(0,nrow=nrow(NDMdata),ncol=ncol(NDMdata))
          p<- -qnorm(alpha)
          for(k in 1:ncol(NDMdata)){
            for(j in 1:ncol(NDMdata)){
              B[,j] = NDMdata[,j] <= upper[,k] & NDMdata[,j] >= lower[,k] & NDMdata[,k]
            }
            a<-apply(B, 1, sum)
            csn <- (B%*%t(B)*n2-a%*%t(a))/sqrt((a%*%t(a))*((n2-a)%*%t(n2-a))/(n2-1))
            csn[is.na(csn)]<-0
            csn1 <- (csn > p)*1
            ndm[,k]<-apply(csn1,1,sum)-diag(csn1)
          }
          rownames(ndm)<-rownames(NDMdata)
          colnames(ndm)<-colnames(NDMdata)
          datareadx<-ndm
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_NDM.table",usertimenum,".csv",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "How to calculate the network degree matrix? Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ndm
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_NDM.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_NDM.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
