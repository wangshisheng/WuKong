...
###########
ui <- dashboardPage(
  title = "Soft independent modelling by class analogy (SIMCA) with LLM Assistance",
  dashboardHeader(title = "Soft independent modelling by class analogy (SIMCA) with LLM Assistance", titleWidth = 760),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                colorPicker(
                  inputId = "pointcolx1",
                  label = h5("3.3. Select colors for each group:"),
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "white"="white",
                    "grey"="grey",
              ...
              numericInput("heightx",h5("3.4. Figure Height:"),550),
            ...
            fluidRow(
              ...
              column(
                ...
                div(style = "display: flex; align-items:left;padding:0px;margin-left: 50px;margin-top: 0px;margin-bottom: 0px;",
                    ...
                    numericInput("heightx2", NULL, 550)
                )
              ),
              column(
                5,
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        SIMCAtextvalue1<<-grnames1
        SIMCAclassname<<-grnames
        SIMCA_leibie_num<<-grnum1
        SIMCA_fenzu_num<<-grnum2
        SIMCA_fenzu_numcum<<-c(0,cumsum(SIMCA_fenzu_num))
        SIMCAdata<<-peaksdataout()
        if(ncol(SIMCAdata)==1){
          datareadx<-data.frame(Description="No Results here. Please upload your dataset or use the example data.")
        }else{
          library(mdatools)
          datareadt<-as.data.frame(t(SIMCAdata))
          simcalist<-list()
          for(i in 1:SIMCA_leibie_num){
            simcadatax<-datareadt[(SIMCA_fenzu_numcum[i]+1):SIMCA_fenzu_numcum[i+1],]
            simcamodelx<-simca(simcadatax,classname = SIMCAtextvalue1[i],center = T,scale = T)
            simcalist[[i]]<-selectCompNum(simcamodelx, 2)
          }
          simcamodellist <- simcam(simcalist)
          simcamodellist1<-mdatools:::plotCooman.simcamres(simcamodellist$res$cal)
          datareadx<-data.frame("Distance to first class"=simcamodellist1$x_values,
                                 "Distance to second class"=simcamodellist1$y_values[,1])
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_SIMCA.table",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(SIMCAdata)==1){
          ...
          simcamodellist <- simcam(simcalist)
          simcamodellist1<-mdatools:::plotCooman.simcamres(simcamodellist$res$cal)
          SIMCAdata1<-data.frame(p1=simcamodellist1$x_values,
                                 p2=simcamodellist1$y_values[,1],
                                 Groups=SIMCAclassname)
          centroids <- aggregate(cbind(p1,p2)~Groups,SIMCAdata1,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(SIMCAdata1$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(SIMCAdata1[SIMCAdata1$Groups==t,1:2]),
                                                      centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                      level=0.95),stringsAsFactors=FALSE)))
          SIMCAdata1$labelnames<-rownames(SIMCAdata1)
          ggplot(data=SIMCAdata1,(aes(x=p1,y=p2,colour = Groups)))+
            geom_vline(xintercept = 0,col="grey",lty=2,lwd=0.5)+
            geom_hline(yintercept = 0,col="grey",lty=2,lwd=0.5)+
            geom_point(size=2)+
            geom_path(data=conf.rgn,size=1)+
            scale_color_manual(values=pointcolx)+
            geom_text(aes(label=labelnames),size=5,hjust=0,vjust=0)+
            labs(x="Distance to first class",y="Distance to second class")+
            theme_bw()+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_SIMCA.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please remove the point labels and make the points bigger. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(data=SIMCAdata1,(aes(x=p1,y=p2,colour = Groups)))+
  ...
  theme(panel.grid.minor = element_blank() ,panel.grid.major = element_blank())
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_SIMCA.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_SIMCA.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
