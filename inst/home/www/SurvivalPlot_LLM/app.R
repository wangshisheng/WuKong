...
###########
ui <- dashboardPage(
  title = "Survival plot with LLM Assistance",
  dashboardHeader(title = "Survival plot with LLM Assistance", titleWidth = 380),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                bsTooltip("grnames",'Enter a formula in the format Surv(time, status) ~ sex (please check the example), where time is the survival time, status indicates the event occurrence (1 for event, 0 for censoring), and sex is the variable to analyze its impact on survival. Please change the variable name based on your own data.',
                          placement = "right",options = list(container = "body"))
              ),
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnames",h5("3.1. Formula:"),value = "Surv(time, status) ~ sex")
              ),
              colorPicker(
                inputId = "colorx1",
...
server <- function(input, output, session) {
    ...
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(survdata)==1){
          ...
          fitx <- survfit(as.formula(formulax), data = survdata)
          ggsurvplot(fitx, data = survdata,palette = colorx)
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Survival",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please add the confidence interval, the p value and the risk table. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
library(survival)
fitx <- survfit(as.formula(formulax), data = survdata)
ggsurvplot(fitx,data = survdata,risk.table = TRUE,pval = TRUE,conf.int = TRUE,
               conf.int.style = "step",xlab = "Time in days",break.time.by = NULL,
               ggtheme = theme_classic(),risk.table.y.text.col = T,
               risk.table.y.text = FALSE,palette=c("blue","red"))
      '
    ...
    output$code_output <- renderUI({
      ...
      #  HTML("")
      #}
      if(is.ggplot(code_result()) | class(code_result())[1]=="ggsurvplot"){
    ...
    output$plot_result <- renderPlot({
      if (!is.null(code_result()) && (is.ggplot(code_result())|class(code_result())[1]=="ggsurvplot")) {
        code_result()
      }
    })
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result()) | class(aax)[1]=="ggsurvplot"){
          paste("Adjusted.by.LLM_Survival.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Survival.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result()) | class(aax)[1]=="ggsurvplot"){
