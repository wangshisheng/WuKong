...
###########
ui <- dashboardPage(
  title = "Gene Set Enrichment Analysis of KEGG with LLM Assistance",
  dashboardHeader(title = "Gene Set Enrichment Analysis of KEGG with LLM Assistance", titleWidth = 680),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              textInput("gseaKEGGwuzhong",h5("3.1. Please type a species:"),value="mmu"),
              selectInput("gseaKEGGidtype",h5("3.2. ID type:"),choices = c("UNIPROT","ENTREZID")),
              #colorPicker(
              #  inputId = "pointcolx",
              ...
              #  multiple = TRUE
              #),
              div(id="gseaKEGGplotcol_div", textInput("gseaKEGGplotindex", h5("3.3. GSEA results index for plotting:"), value = c("1,2,5"))),
              bsTooltip("gseaKEGGplotcol_div", "Enter '1' here to plot the first enrichment result, '1,2' to plot the first one and the second one, and so on. If you enter '1-3', it means plotting the first to the third results on one plot!", options = list(container = "body")),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    dataread<-read.csv("hytestrawdatafc.csv",header = T,stringsAsFactors = F)
    dataread
  })
  ...
  goenrichdefaulttableout<-reactive({
    pronamevecx<<-goenrichplotdataout()[[1]]
    if(length(grep("^No data",pronamevecx))>0){
      ...
      library(dplyr)
      library(tidyr)
      pronamevec<-goenrichplotdataout()[[2]]
      names(pronamevec)<-goenrichplotdataout()[[1]]
      pronamevec<<-sort(pronamevec,decreasing = TRUE)
      idselect<<-input$gseaKEGGwuzhong
      gseaKEGGidtypex<<-input$gseaKEGGidtype
      if(gseaKEGGidtypex=="ENTREZID"){
        idtypex<-"ncbi-geneid"
      }else{
        idtypex<-"uniprot"
      }
      gseaKEGGres<-gseKEGG(geneList=pronamevec,organism=idselect,minGSSize=3,
                           maxGSSize=500,pvalueCutoff=1,keyType=idtypex)
      gseaKEGGresdf1<-gseaKEGGres@result
      datareadx<-gseaKEGGresdf1[,-c(8,10)]
      rownames(datareadx)<-1:nrow(datareadx)
      datareadx
    }
    list(datareadx=datareadx,gseaKEGGres=gseaKEGGres)
  })
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderDataTable({
        allRes.df<<-goenrichdefaulttableout()$datareadx
        if(length(grep("^No data",allRes.df[[1]][1]))>0){
          ...
          allRes.df[[1]] <- paste0("<a href='http://www.genome.jp/dbget-bin/www_bget?pathway+",allRes.df[[1]],"' target='_blank'>",allRes.df[[1]],"</a>")
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_gseaKEGG.table",usertimenum,".csv",sep="")},
        content = function(file){
          write.csv(goenrichdefaulttableout()$datareadx,file,row.names = F)
        }
      )
      output$ClusterCorNetplot<-renderPlot({
        allRes.df1<<-goenrichdefaulttableout()$datareadx
        if(length(grep("^No data",allRes.df1[[1]][1]))>0){
          ...
          library(ggplot2)
          library(enrichplot)
          gseaKEGGplotindexx<<-isolate(input$gseaKEGGplotindex)
          parse_gseaKEGGplotindex <- function(index_string) {
            parts <- unlist(strsplit(index_string, ","))
            result <- c()
            for (part in parts) {
              if (grepl("-", part)) {
                range_limits <- as.numeric(unlist(strsplit(part, "-")))
          ...
          parsed_result <- parse_gseaKEGGplotindex(gseaKEGGplotindexx)
          gseaKEGGresx<<-goenrichdefaulttableout()$gseaKEGGres
          gseaplot2(gseaKEGGresx,geneSetID=parsed_result,pvalue_table=TRUE)
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_gseaKEGG.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked2, {
    ...
    updateTextInput(session, "user_input", value = "Plot: Please remove the pvalue table.")
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    ...
    if(length(grep("^Text:?",user_inputx,ignore.case = T))>0){
      ...
      allRes.df<<-goenrichdefaulttableout()$datareadx
      allRes.dfx<-allRes.df[allRes.df$pvalue<=0.25,]
      #allRes.dfx1<-paste0(allRes.dfx$GOIDs,": ",allRes.dfx$Description,", ",
      #                    allRes.dfx$Ontology,", ",allRes.dfx$IDs,collapse = "; ")
      allRes.dfx1<-paste0(allRes.dfx$ID,": ",allRes.dfx$Description,", ",
                          allRes.dfx$NES,", ",allRes.dfx$core_enrichment,collapse = "; ")
      user_message<-paste0("Please review and learn the following KEGG data entries. Each entry contains the following fields: KEGG id, KEGG description, normalized enrichment score (NES), protein/gene ids. Every KEGG content is pasted with a semicolon: ",allRes.dfx1,". ",input$user_input)
    }else if(length(grep("^Plot:?|Refer to inner codes(\\.)?|=>",user_inputx,ignore.case = T))>0){
      gseaKEGGresult<<-goenrichdefaulttableout()$gseaKEGGres
      typetext1<-'
        library(enrichplot)
        gseaplot2(gseaKEGGresult,geneSetID=c(1,5),color = "green",subplots = 1:3,ES_geom = "line",pvalue_table=F)
        '
    ...
    new_chat <- lapply(chat_history(), function(x) {
      if (x$role == "user") {
        paste0(
          ...
          convert_to_html(gsub("Please review and learn the following KEGG data entries. Each entry contains the following fields: KEGG id, KEGG description, normalized enrichment score \\(NES\\), protein/gene ids. Every KEGG content is pasted with a semicolon: .* Text: ","",
    ...
    output$code_output <- renderUI({
      ...
      #  HTML("")
      #}
      if(is.ggplot(code_result())|class(code_result())[1]=="gglist"){
    ...
    output$plot_result <- renderPlot({
      if (!is.null(code_result()) && (is.ggplot(code_result())|class(code_result())[1]=="gglist")) {
        code_result()
      }
    })
    ...
    output$goenrichllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())|class(code_result())[1]=="gglist"){
          paste("Adjusted.by.LLM_gseaKEGG.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_gseaKEGG.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())|class(code_result())[1]=="gglist"){
