...
###########
ui <- dashboardPage(
  title = "t-SNE with LLM Assistance",
  dashboardHeader(title = "t-SNE with LLM Assistance", titleWidth = 350),
  dashboardSidebar(
    sidebarMenu(
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        tsnetextvalue1<<-grnames1
        tsneclassname<<-grnames
        tsne_leibie_num<<-grnum1
        tsne_fenzu_num<<-grnum2
        tsnedata<<-peaksdataout()
        if(ncol(tsnedata)==1){
          ggplot() + 
          ...
          library(ellipse)
          set.seed(42)
          datareadt<-Rtsne(t(tsnedata),perplexity=2,pca=F,pca_center=F,pca_scale=F)
          tsnedf<-as.data.frame(datareadt$Y)
          tsnedf$Groups<-tsneclassname
          colnames(tsnedf)[1:2]<-c("p1","p2")
          centroids <- aggregate(cbind(p1,p2)~Groups,tsnedf,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(tsnedf$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(tsnedf[tsnedf$Groups==t,1:2]),
                                                               centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                               level=0.95),stringsAsFactors=FALSE)))
          tsnedf$labelnames<-colnames(tsnedata)
          ggplot(data=tsnedf,(aes(x=p1,y=p2,colour = Groups)))+
            geom_vline(xintercept = 0,col="grey",lty=2,lwd=1)+
            geom_hline(yintercept = 0,col="grey",lty=2,lwd=1)+
            geom_point(size=isolate(input$labelsizex))+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_tsne.plot",usertimenum,".pdf",sep="")},
        content = function(file){
...
ggplot(data=tsnedf,(aes(x=p1,y=p2,colour = Groups)))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_tsne.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_tsne.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
