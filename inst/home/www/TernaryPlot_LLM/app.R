...
###########
ui <- dashboardPage(
  title = "Ternary plot with LLM Assistance",
  dashboardHeader(title = "Ternary plot with LLM Assistance", titleWidth = 380),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        Ternarydata<<-peaksdataout()
        if(ncol(Ternarydata)==1){
          ggplot() + 
          ...
          library(ggtern)
          colnames(Ternarydata)<-c("X1","X2","X3","Size","Groups")
          print(ggtern(data = Ternarydata, aes(X1, X2, X3)) +
                  geom_point(aes(size = Size,color=Groups),alpha = 0.5)+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Ternary",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please add the 95% confidence interval. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
print(ggtern(data = Ternarydata, aes(X1, X2, X3)) +
          geom_point(aes(size = Size,color=Groups),alpha = 0.5)+
          scale_color_manual(values = c("#4DBBD5FF","#3C5488FF","#DC0000FF"))+
          theme_rgbw()+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Ternary.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Ternary.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
