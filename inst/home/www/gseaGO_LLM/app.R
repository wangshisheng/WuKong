...
###########
ui <- dashboardPage(
  title = "Gene Set Enrichment Analysis of Gene Ontology with LLM Assistance",
  dashboardHeader(title = "Gene Set Enrichment Analysis of Gene Ontology with LLM Assistance", titleWidth = 680),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              selectInput("gseaGOwuzhong",h5("3.1. Please choose a species:"),choices = c("Mouse","Human","Rat")),
              selectInput("gseaGOidtype",h5("3.2. ID type:"),choices = c("UNIPROT","ENTREZID")),
              #colorPicker(
              #  inputId = "pointcolx",
              ...
              #  multiple = TRUE
              #),
              div(id="gseaGOplotcol_div", textInput("gseaGOplotindex", h5("3.3. GSEA results index for plotting:"), value = c("1,2,5"))),
              bsTooltip("gseaGOplotcol_div", "Enter '1' here to plot the first enrichment result, '1,2' to plot the first one and the second one, and so on. If you enter '1-3', it means plotting the first to the third results on one plot!", options = list(container = "body")),
...
server <- function(input, output, session) {
  ...
  goenrichdefaulttableout<-reactive({
    ...
    if(length(grep("^No data",pronamevecx))>0){
      ...
      idselect<<-input$gseaGOwuzhong
      if(idselect=="Mouse"){
        wuzhongx<-"org.Mm.eg.db"
      }else if(idselect=="Human"){
        wuzhongx<-"org.Hs.eg.db"
      }else{
        wuzhongx<-"org.Rn.eg.db"
      }
      gseaGOidtypex<<-input$gseaGOidtype
      gseagores<<-gseGO(pronamevec,ont="ALL",OrgDb=wuzhongx,keyType=gseaGOidtypex,
                        minGSSize=3,maxGSSize=500,pvalueCutoff=1)
      gseagoresdf1<-gseagores@result
      datareadx<-gseagoresdf1[,c(2,1,3:8,10,12)]
      rownames(datareadx)<-1:nrow(datareadx)
      datareadx
    }
    list(datareadx=datareadx,gseagores=gseagores)
  })
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_gseaGO.table",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(length(grep("^No data",allRes.df1[[1]][1]))>0){
          ...
          library(ggplot2)
          library(enrichplot)
          gseaGOplotindexx<<-isolate(input$gseaGOplotindex)
          parse_gseaGOplotindex <- function(index_string) {
          ...
          parsed_result <- parse_gseaGOplotindex(gseaGOplotindexx)
          gseagoresx<<-goenrichdefaulttableout()$gseagores
          gseaplot2(gseagoresx,geneSetID=parsed_result,pvalue_table=TRUE)
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_gseaGO.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$send, {
    ...
    if(length(grep("^Text:?",user_inputx,ignore.case = T))>0){
      ...
      allRes.df<<-goenrichdefaulttableout()$datareadx
      allRes.dfx<-allRes.df[allRes.df$p.adjust<=0.25,]
      ...
      allRes.dfx1<-paste0(allRes.dfx$ID,": ",allRes.dfx$Description,", ",
                          allRes.dfx$ONTOLOGY,", ",allRes.dfx$NES,", ",allRes.dfx$core_enrichment,collapse = "; ")
      user_message<-paste0("Please learn below contents as I tell you. Below are GO contents (GO id, GO description, GO category, normalized enrichment score (NES), protein uniprot ids), every GO content is pasted with a semicolon: ",allRes.dfx1,". ",input$user_input)
    }else if(length(grep("^Plot:?|Refer to inner codes(\\.)?|=>",user_inputx,ignore.case = T))>0){
      gseagoresult<<-goenrichdefaulttableout()$gseagores
      typetext1<-'
        library(enrichplot)
        gseaplot2(gseagoresult,geneSetID=c(1,5),color = "green",subplots = 1:3,ES_geom = "line",pvalue_table=F)
        '
    ...
    new_chat <- lapply(chat_history(), function(x) {
      if (x$role == "user") {
        paste0(
          ...
          convert_to_html(gsub("Please learn below contents as I tell you. Below are GO contents \\(GO id, GO description, GO category, normalized enrichment score \\(NES\\), protein uniprot ids\\), every GO content is pasted with a semicolon: .* Text: ","",
    ...
    output$goenrichllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())|class(code_result())[1]=="gglist"){
          paste("Adjusted.by.LLM_gseaGO.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_gseaGO.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())|class(code_result())[1]=="gglist"){
