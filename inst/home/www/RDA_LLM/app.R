...
###########
ui <- dashboardPage(
  title = "Redundancy analysis with LLM Assistance",
  dashboardHeader(title = "Redundancy analysis with LLM Assistance", titleWidth = 460),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==1",
                textInput("grnums",h5("3.1. Y variables index (Response variables index):"),value = ""),
                bsTooltip("grnums",'Type in the Y variables index, for example, in the example data, column 1 to column 44 are Y variables (Response variables), users should type in 1-44.',
                          placement = "right",options = list(container = "body")),
                textInput("grnames",h5("3.2. X variables index (Explanatory variables index):"),value = ""),
                bsTooltip("grnames",'Type in the X variables index, for example, in the example data, column 45 to column 58 are X variables (Explanatory variables), users should type in 45-58.',
                          placement = "right",options = list(container = "body"))
              ),
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnums",h5("3.1. Y variables index (Response variables index):"),value = "1-44"),
                textInput("examgrnames",h5("3.2. X variables index (Explanatory variables index):"),value = "45-58")
              ),
              colorPicker(
                inputId = "colorx1",
                label = h5("3.3. Select a color for points:"),
                choices = list(
                  "npg"=pal_npg("nrc")(10),
                  "white"="white",
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    ...
    data(varespec)
    data(varechem)
    dataread<-cbind(varespec,varechem)
    dataread
  })
    ...
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(rdadata)==1){
          ...
          if(input$loaddatatype==1){
            grnumy<-as.numeric(strsplit(isolate(input$grnums),"-")[[1]])
            grnumx<-as.numeric(strsplit(isolate(input$grnames),"-")[[1]])
          }else{
            grnumy<-as.numeric(strsplit(isolate(input$examgrnums),"-")[[1]])
            grnumx<-as.numeric(strsplit(isolate(input$examgrnames),"-")[[1]])
          }
          grnumy<<-grnumy
          grnumx<<-grnumx
          colorx<<-isolate(input$colorx1)
          library(ggvegan)
          ydata<-rdadata[,c(grnumy[1]:grnumy[2])]
          xdata<-rdadata[,c(grnumx[1]:grnumx[2])]
          #hellinger transform
          ydata.hell <- decostand(ydata, "hellinger")
          #Redundancy Analysis
          otu.tab<- rda(ydata.hell ~ ., xdata)
          #Verifing multicollinearity
          vif.cca(otu.tab)
          #The significance of RDA
          anova.cca(otu.tab)
          #Axis significance can be done by adding by = "axis"
          anova.cca(otu.tab, by = "axis")
          #Terms significance can be done by adding by = "terms"
          anova.cca(otu.tab, by = "terms")
          #plot
          autoplot(otu.tab, arrows = TRUE,axes = c(1, 2), geom = "point",
                                legend.position = "right", title = "RDA")+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_RDA",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please change the point size bigger. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    if(input$loaddatatype==1){
      grnumy<-as.numeric(strsplit(input$grnums,"-")[[1]])
      grnumx<-as.numeric(strsplit(input$grnames,"-")[[1]])
    }else{
      grnumy<-as.numeric(strsplit(input$examgrnums,"-")[[1]])
      grnumx<-as.numeric(strsplit(input$examgrnames,"-")[[1]])
    }
    grnumy<<-grnumy
    grnumx<<-grnumx
    rdadata<<-peaksdataout()
...
anova.cca(otu.tab, by = "terms")
#plot
obj <- fortify(otu.tab, axes = c(1, 2))
autoplot(otu.tab, arrows = TRUE,axes = c(1, 2), geom = "text",
      legend.position = "right", title = "RDA")+
      geom_point(data = obj[obj[["score"]] %in% "sites",],aes(RDA1,RDA2,colour = score),size=4)+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_RDA.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_RDA.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
