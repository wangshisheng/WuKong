...
###########
ui <- dashboardPage(
  title = "PLS-DA with LLM Assistance",
  dashboardHeader(title = "PLS-DA with LLM Assistance", titleWidth = 350),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderPrint({
        ...
        plsdatextvalue1<<-grnames1
        plsdaclassname<<-grnames
        plsda_leibie_num<<-grnum1
        plsda_fenzu_num<<-grnum2
        plsdadata<<-peaksdataout()
        if(ncol(plsdadata)==1){
          ...
          library(ellipse)
          library(ropls)
          datareadt<-t(plsdadata)
          plsdaYY<-plsdaclassname
          plsdares<-opls(datareadt, plsdaYY, scaleC="standard",fig.pdfC="none",info.txtC="none", crossvalI=6)
          #show the summary results of plsda model
          datareadx<-plsdares@summaryDF
        }
        datareadx
      })
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(plsdadata)==1){
          ...
          #show the summary results of plsda model
          plsdares@summaryDF
          #show the score point plot of plsda model
          plsdadf<-as.data.frame(plsdares@scoreMN)
          plsdadf$Groups<-plsdaclassname
          centroids <- aggregate(cbind(p1,p2)~Groups,plsdadf,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(plsdadf$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(plsdadf[plsdadf$Groups==t,1:2]),
                                                               centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                               level=0.95),stringsAsFactors=FALSE)))
          plsdadf$labelnames<-rownames(plsdadf)
          ggplot(data=plsdadf,(aes(x=p1,y=p2,colour = Groups)))+
            ...
            labs(x=paste0("t1 (",plsdares@modelDF[1,1]*100,"%)"), 
                 y = paste0("t2 (",plsdares@modelDF[2,1]*100,"%)"))+
            theme_bw()+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_plsda.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "I want all point size are bigger and remove the point labels. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_plsda.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_plsda.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
