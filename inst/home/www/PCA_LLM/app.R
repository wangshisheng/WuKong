...
###########
ui <- dashboardPage(
  title = "Principal Component Analysis (PCA) with LLM Assistance",
  dashboardHeader(title = "Principal Component Analysis (PCA) with LLM Assistance", titleWidth = 580),
  dashboardSidebar(
    sidebarMenu(
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderPrint({
        ...
        pcatextvalue1<<-grnames1
        pcaclassname<<-grnames
        pca_leibie_num<<-grnum1
        pca_fenzu_num<<-grnum2
        pcadata<<-peaksdataout()
        if(ncol(pcadata)==1){
          ...
          #show the summary results of PCA model
          datareadx<-prdataread$eig
        }
        datareadx
      })
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(pcadata)==1){
          ...
          labelsizex<<-input$labelsizex
          library(factoextra)
          library(FactoMineR)
          prdataread<-PCA(t(pcadata),scale.unit = TRUE, ncp = ncol(pcadata),graph = FALSE)
          fviz_pca_ind(prdataread,
                       label = "none",
                       habillage = factor(pcaclassname,levels = pcatextvalue1),
                       palette = pointcolx,
                       mean.point = FALSE,
                       pointshape="circle",pointsize = labelsizex,
                       addEllipses = TRUE)+
            theme_bw()+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_PCA.plot",usertimenum,".pdf",sep="")},
        content = function(file){
...
fviz_pca_ind(prdataread,
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_PCA.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_PCA.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
