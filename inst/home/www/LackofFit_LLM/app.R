...
###########
ui <- dashboardPage(
  title = "Lack of Fit F-test with LLM Assistance",
  dashboardHeader(title = "Lack of Fit F-test with LLM Assistance", titleWidth = 430),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnames",h5("3.1. Formula:"),value = "y~x1+x2")
              ),
              hr(),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    ...
    y <- rnorm(10, x1+x2)
    dataread<-data.frame(y=y,x1=x1,x2=x2)
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      output$lackoffitplot<-renderPrint({
        ...
        if(ncol(lackoffitdata)==1){
          'No uploaded data and No plot here.\n Please upload data or click the example data.'
        }else{
          library(alr3)
          fit <- lm(as.formula(lackoffitformulax), data=lackoffitdata)
          pureErrorAnova(fit)
        }
      })
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "How to interprate the results. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
pureErrorAnova(fit) \n
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_lackoffit.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_lackoffit.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
