...
###########
ui <- dashboardPage(
  title = "Diverging Barplot with LLM Assistance",
  dashboardHeader(title = "Diverging Barplot with LLM Assistance", titleWidth = 400),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              numericInput("heightx",h5("3.2. Figure Height:"),900),
              numericInput("widthx",h5("3.3. Figure Width:"),750),
              hr(),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    ...
    dataread<-as.data.frame(scale(dataread)[21:30,1,drop=F])
    dataread$Names<-rownames(dataread)
    dataread<-dataread[,c(2,1)]
    colnames(dataread)<-c("Names","Values")
    rownames(dataread)<-1:nrow(dataread)
    dataread
  })
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$DivergingBarplot<-renderPlot({
        ...
        DivergingBardata<<-peaksdataout()
        if(ncol(DivergingBardata)==1){
          ggplot() + 
          ...
          library(ggplot2)
          colnames(DivergingBardata)<-c("Names","Values")
          DivergingBardata$Groups <- ifelse(DivergingBardata$Values < 0, "Negative", "Positive")
          DivergingBardata1<-DivergingBardata[order(DivergingBardata$Values), ]
          DivergingBardata1$Names <- factor(DivergingBardata1$Names, levels = DivergingBardata1$Names)
          ggplot(DivergingBardata1, aes(x=Names, y=Values, label=Names)) + 
            geom_bar(stat='identity', aes(fill=Groups), width=.5)  +
            scale_fill_manual(values = c("Negative"=colorx[1], "Positive"=colorx[2])) + 
            coord_flip()+
            theme_bw()
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_DivergingBar.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Change the bar color to red and blue. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(DivergingBardata1, aes(x=Names, y=Values, label=Names))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DivergingBar.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DivergingBar.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
