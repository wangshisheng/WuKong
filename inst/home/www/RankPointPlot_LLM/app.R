...
###########
ui <- dashboardPage(
  title = "Rank Point Plot with LLM Assistance",
  dashboardHeader(title = "Rank Point Plot with LLM Assistance", titleWidth = 430),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              colorPicker(
                inputId = "colorx1",
                label = h5("3.1. Select a color for points:"),
                choices = list(
                  "npg"=pal_npg("nrc")(10),
                  "white"="white",
              ...
              checkboxInput('logifx', '3.2. Log2 transformation?', TRUE),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    library(writexl)
    dataread<-read.csv("prorankdf.csv",stringsAsFactors = F,check.names = F)#,row.names = 1
    #dataread[[2]]<-log2(dataread[[2]])
    dataread
  })
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        RankPointPlotdata<<-peaksdataout()
        if(ncol(RankPointPlotdata)==1){
          ggplot() + 
          ...
          if(input$logifx){
            RankPointPlotdata[[2]]<-log2(RankPointPlotdata[[2]])
          }else{
            RankPointPlotdata<-RankPointPlotdata
          }
          colorx<<-isolate(input$colorx1)
          library(ggplot2)
          RankPointPlotdata1<-data.frame(X=1:nrow(RankPointPlotdata),
                                         Value=sort(RankPointPlotdata[[2]],decreasing = T))
          ggplot(RankPointPlotdata1,aes(x=X,y=log2(Value)))+
            geom_point(color=colorx)+
            xlab("Protein Number")+ylab("Intensity (log2)")+
            theme_classic()
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_RankPoint",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please highlight the proteins below, Refer to inner codes:\nP61978\nP12956\nQ02952\n")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
library(ggrepel)
library(ggplot2)
RankPointPlotdata1<-RankPointPlotdata[order(RankPointPlotdata[[2]],decreasing = T),]
# Define proteins to highlight
highlight_proteins <- c("P61978", "P12956", "Q02952")
# Add a column to mark highlighted proteins
RankPointPlotdata1$highlight <- ifelse(RankPointPlotdata1[[1]] %in% highlight_proteins, "Highlighted", "Other")
RankPointPlotdata1$X<-1:nrow(RankPointPlotdata1)
RankPointPlotdata1$Values<-RankPointPlotdata1[[2]]
RankPointPlotdata1$Names<-RankPointPlotdata1[[1]]
ggplot(RankPointPlotdata1, aes(x = X, y = log2(Values), color = highlight)) +
    geom_point() +
    scale_color_manual(values = c("Highlighted" = "red", "Other" = "grey")) +
    geom_text_repel(data = subset(RankPointPlotdata1, highlight == "Highlighted"),
                    aes(label = Names),
                    color = "red") +
    xlab("Protein Number") +
    ylab("Intensity (log2)") +
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_RankPoint.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_RankPoint.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
