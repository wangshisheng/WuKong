...
###########
ui <- dashboardPage(
  title = "Nightingale Rose plot with LLM Assistance",
  dashboardHeader(title = "Nightingale Rose plot with LLM Assistance", titleWidth = 480),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              colorPicker(
                ...
                selected=c("#3C5488FF","#8491B4FF","#DC0000FF"),
                plainColor = T,
                multiple = TRUE
              ),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        Nightingaledata<<-peaksdataout()
        if(ncol(Nightingaledata)==1){
          ggplot() + 
          ...
          library(ggplot2)
          Nightingaledata<-Nightingaledata[order(Nightingaledata[[2]],decreasing = FALSE),]
          colnames(Nightingaledata)<-c("Names","Number")
          Nightingaledata$id<-1:nrow(Nightingaledata)
          label_data <- Nightingaledata
          number_of_bar <- nrow(label_data)
          angle <-  90 - 360 * (label_data$id-0.5)/number_of_bar
          label_data$hjust<-ifelse( angle < -90, 1, 0)
          label_data$angle<-ifelse(angle < -90, angle+180, angle)
          ggplot(data = Nightingaledata, aes(x = id, y = Number, label = Names)) +
            geom_col(aes(fill = Number), width = 1, size = 0) +
            geom_col(
              aes(y = 500),fill = "white",width = 1,alpha = 0.4,size = 0
            ) +
            geom_col(
              aes(y = 300),fill = "white",width = 1,alpha = 0.4,size = 0
            )+
            ...
            coord_polar(start = 0) +
            theme_void() +
            geom_text(data=label_data, aes(x=id, y=Number+5, label=Names, hjust=hjust), 
                      color="black", size=2, 
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Nightingale",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please add Number labels on the plot. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(data = Nightingaledata, aes(x = id, y = Number, label = Names)) +
      geom_col(aes(fill = Number), width = 1, size = 0) +
      geom_col(aes(y = 500),fill = "white",width = 1,alpha = 0.4,size = 0) +
      geom_col(aes(y = 300),fill = "white",width = 1,alpha = 0.4,size = 0)+
      scale_fill_gradientn(colors = c("#54778f","#E69D2A","#A61650"))+
      coord_polar(start = 0) +
      theme_void() +
      geom_text(data=label_data, aes(x=id, y=Number+5, label=paste0(Names,": ",Number), hjust=hjust), 
      color="black", size=2, angle= label_data$angle, inherit.aes = FALSE)
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Nightingale.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Nightingale.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
