...
###########
ui <- dashboardPage(
  title = "Violin Plot with LLM Assistance",
  dashboardHeader(title = "Violin Plot with LLM Assistance", titleWidth = 360),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        Violinptextvalue1<<-grnames1
        Violinpclassname<<-grnames
        Violinp_leibie_num<<-grnum1
        Violinp_fenzu_num<<-grnum2
        Violinpdata<<-peaksdataout()
        if(ncol(Violinpdata)==1){
          ggplot() + 
          ...
          library(reshape2)
          library(ggforce)
          Violinpdata1<-melt(Violinpdata)
          Violinpdata1$Group<-rep(Violinpclassname,each=nrow(Violinpdata))
          ggplot(Violinpdata1, aes(x = variable, y = value,color = Group)) +
            #scale_color_brewer(palette = "Dark2", guide = "none")+
            scale_color_manual(values = pointcolx)+
            geom_violin(fill = "gray80", linewidth = 1, alpha = .8)+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Violin.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please change the transparency to 0. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
Violinpdata1<-melt(Violinpdata)
Violinpdata1$Group<-rep(Violinpclassname,each=nrow(Violinpdata))
ggplot(Violinpdata1, aes(x = variable, y = value,
      color = Group)) +
      scale_color_brewer(palette = "Dark2", guide = "none")+
      geom_violin(fill = "gray80", linewidth = 1, alpha = 0)+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Violin.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Violin.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
