...
###########
ui <- dashboardPage(
  title = "Colored scatterplot with LLM Assistance",
  dashboardHeader(title = "Colored scatterplot with LLM Assistance", titleWidth = 410),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        heatscatterdata<<-peaksdataout()
        if(ncol(heatscatterdata)==1){
          ggplot() + 
          ...
          library(LSD)
          colpalx<-colorpalette(colorx,10)
          pp<-heatscatter(heatscatterdata[[1]],heatscatterdata[[2]], main="",colpal=colpalx, nrcol =100,
                          xlab="X",ylab="Y",ggplot =TRUE)
          pp
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Scatterplot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Change the color to spectral. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    ...
    if(length(user_inputx1)>0){
      typetext1<-'
library(LSD)
pp<-heatscatter(heatscatterdata[[1]],heatscatterdata[[2]], main="",colpal="spectral", nrcol =100,xlab="X",ylab="Y",ggplot =TRUE)
pp
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Scatterplot.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Scatterplot.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
