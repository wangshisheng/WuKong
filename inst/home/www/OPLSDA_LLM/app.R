...
###########
ui <- dashboardPage(
  title = "OPLS-DA with LLM Assistance",
  dashboardHeader(title = "OPLS-DA with LLM Assistance", titleWidth = 350),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              radioButtons(
                "resultsout",
                label = NULL,
                choices = list("4.1. Visualization" = 1,"4.2. Results summary"=2),
                selected = 1,
                inline = TRUE
              ),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderPrint({
        ...
        oplsdatextvalue1<<-grnames1
        oplsdaclassname<<-grnames
        oplsda_leibie_num<<-grnum1
        oplsda_fenzu_num<<-grnum2
        oplsdadata<<-peaksdataout()
        if(ncol(oplsdadata)==1){
          ...
          library(ellipse)
          library(ropls)
          datareadt<-t(oplsdadata)
          oplsdaYY<-oplsdaclassname
          oplsdares<-opls(datareadt, oplsdaYY, scaleC="standard",fig.pdfC="none",info.txtC="none", 
                          crossvalI=6, predI = 1, orthoI = NA)
          #show the summary results of oplsda model
          datareadx<-oplsdares@summaryDF
        }
        datareadx
      })
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(oplsdadata)==1){
          ...
          #show the summary results of oplsda model
          oplsdares@summaryDF
          #show the score point plot of oplsda model
          oplsdadf<-data.frame(t1=oplsdares@scoreMN[,1],to1=oplsdares@orthoScoreMN[,1])
          oplsdadf$Groups<-oplsdaclassname
          centroids <- aggregate(cbind(t1,to1)~Groups,oplsdadf,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(oplsdadf$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(oplsdadf[oplsdadf$Groups==t,1:2]),
                                                               centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                               level=0.95),stringsAsFactors=FALSE)))
          oplsdadf$labelnames<-rownames(oplsdadf)
          ggplot(data=oplsdadf,(aes(x=t1,y=to1,colour = Groups)))+
            geom_vline(xintercept = 0,col="grey",lty=2,lwd=1)+
            geom_hline(yintercept = 0,col="grey",lty=2,lwd=1)+
            geom_point(size=input$labelsizex)+
            geom_path(data=conf.rgn,size=1)+
            scale_color_manual(values=pointcolx)+
            geom_text(aes(label=labelnames),size=5,hjust=0,vjust=0)+
            labs(x=paste0("t1 (",oplsdares@modelDF[1,1]*100,"%)"), 
                 y = paste0("to1 (",oplsdares@modelDF[2,1]*100,"%)"))+
            theme_bw()+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_oplsda.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please remove the point labels. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(data=oplsdadf,(aes(x=t1,y=to1,colour = Groups)))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_oplsda.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_oplsda.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
