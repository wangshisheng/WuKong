...
###########
ui <- dashboardPage(
  title = "Linear Regression with LLM Assistance",
  dashboardHeader(title = "Linear Regression with LLM Assistance", titleWidth = 430),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnames",h5("3.1. Formula:"),value = "y~x")
              ),
              hr(),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    ...
    dataread<-log2(dataread[1:100,1:2])
    ...
    input$mcsbtn_Barplot,{
      output$lackoffitplot<-renderPrint({
        ...
        if(ncol(LinearRegpdata)==1){
          ...
          #colnames(LinearRegpdata) <- c("x","y")
          #Show the linear regression summary results
          LinearRegres<-lm(as.formula(lackoffitformulax),data=LinearRegpdata)
          LinearRegres1<-summary(LinearRegres)
          print(LinearRegres1)
        }
      })
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Show the scatter plot with the linear regression results. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    ...
    lackoffitdata<<-peaksdataout()
    user_inputx<<-input$user_input
    user_inputx1<-grep("Refer to inner codes(\\.)?|=>",user_inputx,ignore.case = T)
    #req(input$user_input)
    if(length(user_inputx1)>0){#colnames(LinearRegpdata) <- c("x","y")
      typetext1<-'
library(psych)
library(ggplot2)
library(ggpmisc)
ggplot(data = LinearRegpdata, aes(x = x, y = y)) +
    stat_poly_line(method="lm",linetype=2,
                   colour="red",se=TRUE,level=0.95,alpha=0.2) +
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_LinearReg.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_LinearReg.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
