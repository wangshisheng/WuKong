...
###########
ui <- dashboardPage(
  title = "Scatters plot with ellipses with LLM Assistance",
  dashboardHeader(title = "Scatters plot with ellipses with LLM Assistance", titleWidth = 480),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnums",h5("3.1. Group and replicate number:"),value = "2;5-5"),
                textInput("examgrnames",h5("3.2. Group names:"),value = "R;C")
              ),
              colorPicker(
                inputId = "colorx1",
                label = h5("3.3. Select colors:"),
                choices = list(
                  "npg"=pal_npg("nrc")(10),
                  "white"="white",
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        Scatter.ellipsetextvalue1<<-grnames1
        Scatter.ellipseclassname<<-grnames
        Scatter.ellipse_leibie_num<<-grnum1
        Scatter.ellipse_fenzu_num<<-grnum2
        Scatter.ellipsedata<<-peaksdataout()
        if(ncol(Scatter.ellipsedata)==1){
          ggplot() + 
          ...
          library(ggpubr)
          library(ellipse)
          Scatter.ellipsedata$Groups<-Scatter.ellipseclassname
          colnames(Scatter.ellipsedata)[c(1,2)]<-c("p1","p2")
          centroids <- aggregate(cbind(p1,p2)~Groups,Scatter.ellipsedata,mean)
          conf.rgn  <- do.call(rbind,lapply(unique(Scatter.ellipsedata$Groups),function(t)
            data.frame(Groups=as.character(t),ellipse::ellipse(cov(Scatter.ellipsedata[Scatter.ellipsedata$Groups==t,1:2]),
                                                               centre=as.matrix(centroids[centroids$Groups==t,2:3]),
                                                               level=0.95),stringsAsFactors=FALSE)))
          Scatter.ellipsedata$labelnames<-rownames(Scatter.ellipsedata)
          ggplot(data=Scatter.ellipsedata,(aes(x=p1,y=p2,colour = Groups)))+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Scatter.ellipse",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please remove point labels. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(data=Scatter.ellipsedata,(aes(x=p1,y=p2,colour = Groups)))+
  ...
  theme(panel.grid.minor = element_blank() ,panel.grid.major = element_blank())
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Scatter.ellipse.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Scatter.ellipse.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
