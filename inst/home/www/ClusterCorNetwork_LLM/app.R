...
###########
ui <- dashboardPage(
  title = "Correlation Network Clustering with LLM Assistance",
  dashboardHeader(title = "Correlation Network Clustering with LLM Assistance", titleWidth = 520),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                numericInput("juleinum",h5("3.1. Cluster number:"),value=NULL),
                numericInput("coryuzhi",h5("3.2. Threshold of correlation coefficient:"),value=NULL),
              ...
              conditionalPanel(
                ...
                numericInput("juleinum",h5("3.1. Cluster number:"),value=4),
                numericInput("coryuzhi",h5("3.2. Threshold of correlation coefficient:"),value=0.9),
                #checkboxInput('scale', '3.6. Standardize the data?',TRUE),
                #checkboxInput('yangbenhao', '3.7. Show sample numbers?', TRUE),
                #numericInput("zitisize","3.8. Font size of the sample numbers in the plot:",value=8)
              ),
              numericInput("heightx",h5("3.3. Figure Height:"),900),
              numericInput("widthx",h5("3.4. Figure Width:"),750),
              hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        ClusterCorNetdata<<-peaksdataout()
        if(ncol(ClusterCorNetdata)==1){
          ggplot() + 
          ...
          library(corrr)
          library(tidyverse)
          juleinumx<-input$juleinum
          coryuzhix<-input$coryuzhi
          ClusterCorNetdata1 <- ClusterCorNetdata %>%
            t() %>% correlate() %>%
            shave(upper = TRUE) %>%
            stretch(na.rm = TRUE) %>%           
            filter(abs(r) >= coryuzhix)
          set.seed(100)
          ClusterCorNet.graph <- as_tbl_graph(ClusterCorNetdata1, directed = FALSE)
          set.seed(100)
          ClusterCorNetdata2<-kmeans(ClusterCorNetdata,juleinumx)
          ClusterCorNetdata3 <- data.frame(
            name = rownames(ClusterCorNetdata),
            group = as.factor(ClusterCorNetdata2$cluster)
          )
          ClusterCorNet.graph <- ClusterCorNet.graph %>%
            activate(nodes) %>%
            left_join(ClusterCorNetdata3, by = "name") %>%
            rename(label = name) %>% 
            activate(edges) %>% 
            rename(weight=r)
          set.seed(100)
          ggraph(ClusterCorNet.graph,layout = "graphopt")+ 
            geom_edge_link(aes(width = abs(weight),color=weight), alpha = 0.2) + 
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_ClusterCorNet",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please use k-means result to the network components. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  ClusterCorNet.graph <- as_tbl_graph(ClusterCorNetdata1, directed = FALSE)
  set.seed(100)
  ClusterCorNetdata2<-kmeans(ClusterCorNetdata,4)
  ClusterCorNetdata3 <- data.frame(
    name = rownames(ClusterCorNetdata),
    group = as.factor(ClusterCorNetdata2$cluster)
  )
  ...
  ggraph(ClusterCorNet.graph,layout = "graphopt")+ 
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_ClusterCorNet.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_ClusterCorNet.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
