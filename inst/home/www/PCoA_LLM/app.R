...
###########
ui <- dashboardPage(
  title = "PCoA with LLM Assistance",
  dashboardHeader(title = "PCoA with LLM Assistance", titleWidth = 350),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              numericInput("labelsizex",h5("3.4. Point size:"),2),
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderPrint({
        ...
        PCoAtextvalue1<<-grnames1
        PCoAclassname<<-grnames
        PCoA_leibie_num<<-grnum1
        PCoA_fenzu_num<<-grnum2
        PCoAdata<<-peaksdataout()
        if(ncol(PCoAdata)==1){
          ...
          library(ade4)
          library(vegan)
          df.dist<-vegdist(t(PCoAdata),method='euclidean')
          pcoa<-dudi.pco(df.dist,scannf = F,nf=2)
          #show the summary results of PCoA model
          datareadx<-summary(pcoa)
        }
        datareadx
      })
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(PCoAdata)==1){
          ...
          pcoa<-dudi.pco(df.dist,scannf = F,nf=2)
          pcoares<-pcoa$li
          pcoares$name<-rownames(pcoares)
          pcoares$group<-PCoAclassname
          ggplot(pcoares,aes(x = A1,y = A2,color = group,group = group,fill = group))+
            ...
            stat_ellipse(aes(x=A1,y=A2),geom = "path",level = 0.95,alpha=1,
                         na.rm=T,linewidth=1)+
            geom_text(aes(label=name),vjust=1.5,size=2)+
            labs(
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_PCoA.plot",usertimenum,".pdf",sep="")},
        content = function(file){
...
ggplot(pcoares,aes(x = A1,y = A2,color = group,group = group,fill = group))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_PCoA.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_PCoA.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
