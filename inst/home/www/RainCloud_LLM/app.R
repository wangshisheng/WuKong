...
###########
ui <- dashboardPage(
  title = "Rain Cloud Plot with LLM Assistance",
  dashboardHeader(title = "Rain Cloud Plot with LLM Assistance", titleWidth = 410),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        RainCloudtextvalue1<<-grnames1
        RainCloudclassname<<-grnames
        RainCloud_leibie_num<<-grnum1
        RainCloud_fenzu_num<<-grnum2
        RainClouddata<<-peaksdataout()
        if(ncol(RainClouddata)==1){
          ggplot() + 
          ...
          library(gghalves)
          library(reshape2)
          RainClouddata1<-melt(RainClouddata)
          RainClouddata1$Group<-rep(RainCloudclassname,each=nrow(RainClouddata))
          ggplot(RainClouddata1, aes(variable, value,fill=Group)) + 
            ggdist::stat_halfeye(adjust = .5, width = .3, .width = 0, justification = -.3, point_colour = NA) + 
            geom_boxplot(width = .1, outlier.shape = NA) +
            gghalves::geom_half_point(aes(color=Group),side = "l", range_scale = .4, 
                                      alpha = .5,transformation = position_jitter(seed = 123))+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_RainCloud.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please increase the point size. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(RainClouddata1, aes(variable, value,fill=Group)) + 
  ggdist::stat_halfeye(adjust = .5, width = .3, .width = 0, justification = -.3, point_colour = NA) + 
  geom_boxplot(width = .1, outlier.shape = NA) +
  gghalves::geom_half_point(aes(color=Group),side = "l", range_scale = .4, size=5,
  alpha = .5,transformation = position_jitter(seed = 123))+
  ...
  panel.grid.major = element_blank())
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_RainCloud.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_RainCloud.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
