...
###########
ui <- dashboardPage(
  title = "Interactive Box and Point plot with LLM Assistance",
  dashboardHeader(title = "Interactive Box and Point plot with LLM Assistance", titleWidth = 500),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                checkboxInput('firstcol', 'Is the first column names?', FALSE),
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==1",
                #textInput("classx1","3.1. Group and replicate number:",value = ""),
                #textInput("classnamex1","3.2. Group names:",value = ""),
                colorPicker(
                  inputId = "colorx1",
                  label = h5("3.1. Select bar colors:"),
                  choices = list("npg"=pal_npg("nrc")(10),
                ...
                #numericInput("pointsize","3.4. Size of the points in the plot:",value=3),
                #numericInput("aixssize","3.5. Font size of the axes in the plot:",value=15),
                #checkboxInput('scale', '3.6. Standardize the data?',TRUE),
                #checkboxInput('yangbenhao', '3.7. Show sample numbers?', TRUE),
                #numericInput("zitisize","3.8. Font size of the sample numbers in the plot:",value=8)
              ),
              conditionalPanel(
                condition = "input.loaddatatype==2",
                #textInput("classx2","3.1. Group and replicate number:",value = "2;5-5"),
                #textInput("classnamex2","3.2. Group names:",value = "R;C"),
                colorPicker(
                  inputId = "colorx2",
                  label = h5("3.1. Select box colors:"),
                  choices = list("npg"=pal_npg("nrc")(10),
                                 "aaas"=pal_aaas("default")(10),
                                 "nejm"=pal_nejm("default")(8),
                                 "pal"=scales::brewer_pal(palette = "Paired")(8)), 
                  selected=c("#E64B35FF","#4DBBD5FF"),
                  plainColor = T,
                  multiple = TRUE
                ),
                ...
                #numericInput("pointsize","3.4. Size of the points in the plot:",value=3),
                #numericInput("aixssize","3.5. Font size of the axes in the plot:",value=15),
                #checkboxInput('scale', '3.2. Standardize the data?',TRUE),
                #checkboxInput('yangbenhao', '3.7. Show sample numbers?', TRUE),
                #numericInput("zitisize","3.8. Font size of the sample numbers in the plot:",value=8)
              ),
              selectInput("hytestx", h5("3.2. Select a test method:"), choices = c("None","t_test","wilcox_test","anova_test","kruskal_test")),
              numericInput("heightx",h5("3.3. Figure Height:"),500),
              numericInput("widthx",h5("3.4. Figure Width:"),500),
              hr(),
          ...
          column(
            ...
            box(
              ...
              radioButtons(
                "resultsout",
                label = NULL,
                choices = list("4.1. Box and Point plot Visualization" = 1),
                selected = 1,
                inline = TRUE
              ),
              ...
              conditionalPanel(
                ...
                div(style = "overflow-x: auto;overflow-y: auto;", plotOutput("DivergingBarplot",height = "550px"))#
              )
            )
          )
        )
...
server <- function(input, output, session) {
    ...
    input$mcsbtn_Barplot,{
      ...
      output$DivergingBarplot<-renderPlot({
        ...
        if(ncol(Bardata1)==1){
          ...
          hytestx<<-isolate(input$hytestx)
          library(ggplot2)
          library(ggpubr)
          library(rstatix)
          Bardata<-reshape2::melt(Bardata1)
          colnames(Bardata)[1]<-"group"
          if(hytestx=="None"){
            ggboxplot(Bardata, x = "group", y = "value",
                      color = "group",palette =colorx,
                      add = "jitter")+
              scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
          }else if(hytestx=="t_test"){
            stat.test <- Bardata %>%
              t_test(value ~ group,p.adjust.method="BH") %>%
              add_xy_position(fun = "mean_se", x = "group", dodge = 0.8)
            ggboxplot(Bardata, x = "group", y = "value",
                      color = "group",palette =colorx,
                      add = "jitter")+
              stat_pvalue_manual(
            ...
            ggboxplot(Bardata, x = "group", y = "value",
              ...
              scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
          }else if(hytestx=="anova_test"){
            #stat.test <- Bardata %>%
            #  anova_test(value ~ group)
            ggboxplot(Bardata, x = "group", y = "value",
                      color = "group",palette =colorx,
                      add = "jitter")+
              stat_compare_means(method = "anova",label.y=ceiling(max(Bardata$value)))+
              #stat_pvalue_manual(
              #  stat.test, label = "p", tip.length = 0.01,
              #  bracket.nudge.y = 2
              #)+
            ...
            ggboxplot(Bardata, x = "group", y = "value",
                      color = "group",palette =colorx,
                      add = "jitter")+
              stat_compare_means(method = "kruskal.test",label.y=ceiling(max(Bardata$value)))+
              #stat_pvalue_manual(
              #  stat.test, label = "p", tip.length = 0.01,
              #  bracket.nudge.y = 2
              #)+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_BoxPoint.plot",usertimenum,".pdf",sep="")},
        content = function(file){
          pdf(file, width = widthx()/96,height = heightx()/96)
          print(DivergingBarplotout())
          dev.off()
        }
      )
    }
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please change the bar color to '#78A1C9' and '#BC5555'. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
library(ggpubr)
library(rstatix)
Boxdata<-reshape2::melt(Boxdata1)
colnames(Boxdata)[1]<-"group"
hytestx<-"None"
if(hytestx=="None"){
    ggboxplot(Boxdata, x = "group", y = "value",
              color = "group",palette =c("#78A1C9", "#BC5555"),
              add = "jitter")+
        scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
}else if(hytestx=="t_test"){
    stat.test <- Boxdata %>%
    ...
    ggboxplot(Boxdata, x = "group", y = "value",
              color = "group",palette =c("#78A1C9", "#BC5555"),
              add = "jitter")+
        stat_compare_means(method = "anova",label.y=ceiling(max(Boxdata$value)))+
        scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
}else{
    ggboxplot(Boxdata, x = "group", y = "value",
              color = "group",palette =c("#78A1C9", "#BC5555"),
              add = "jitter")+
        stat_compare_means(method = "kruskal.test",label.y=ceiling(max(Boxdata$value)))+
        scale_y_continuous(expand = expansion(mult = c(0, 0.1)))
}
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_BoxPoint.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_BoxPoint.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
