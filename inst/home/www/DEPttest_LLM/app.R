...
###########
ui <- dashboardPage(
  title = "Student's t-Test with LLM Assistance",
  dashboardHeader(title = "Student's t-Test with LLM Assistance", titleWidth = 380),
  dashboardSidebar(
    sidebarMenu(
    ...
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        if(ncol(DEPdata)==1){
          ...
          P.Value<-apply(DEPdata, 1, function(x) t.test(x[1:DEP_fenzu_num[1]], x[(DEP_fenzu_num[1]+1):ncol(DEPdata)],alternative="two.sided",mu=0,paired=FALSE,var.equal = FALSE)$p.value)
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_DEPTtest.table",usertimenum,".csv",sep="")},
        content = function(file){
...
p.adjust<-p.adjust(P.Value,method = "BH")
FCx<-rowMeans(DEPdata[,c((DEP_fenzu_num[1]+1):ncol(DEPdata))],na.rm = T)-rowMeans(DEPdata[,c(1:DEP_fenzu_num[1])],na.rm = T)
Ttesttabledf<-cbind(DEPdata,Fold.Change=FCx,p.Value=P.Value,p.adjust=p.adjust)
Ttesttabledf
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DEPTtest.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DEPTtest.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
