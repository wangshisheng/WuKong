...
###########
ui <- dashboardPage(
  title = "Contour Plot with LLM Assistance",
  dashboardHeader(title = "Contour Plot with LLM Assistance", titleWidth = 400),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    ...
    dataread<-log2(dataread[1:500,c(1:3)])
    colnames(dataread)<-c("x","y","z")
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$DivergingBarplot<-renderPlot({
        ...
        Contourpdata<<-peaksdataout()
        if(ncol(Contourpdata)==1){
          ggplot() + 
          ...
          library(akima)
          #colnames(Contourpdata)<-c("x","y","z")
          Contourpdata1 <- with(Contourpdata, akima::interp(x = x, y = y, z = z))
          df <- Contourpdata1$z |>
          ...
          ggplot(data = df, aes(x = x, y = y, z = z)) +
            ...
            scale_fill_gradient(low = colorx[1], high = colorx[2], na.value = NA)+
            theme_bw()
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Contour.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please change the color for low values to 'yellow' and for high values to 'red'. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(data = df, aes(x = x, y = y, z = z)) +
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Contour.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Contour.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
