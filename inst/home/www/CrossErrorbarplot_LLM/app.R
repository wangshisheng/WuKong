...
###########
ui <- dashboardPage(
  title = "Cross Error Bar plot with LLM Assistance",
  dashboardHeader(title = "Cross Error Bar plot with LLM Assistance", titleWidth = 480),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                colorPicker(
                  inputId = "pointcolx1",
                  label = h5("3.3. Select colors for each point:"),
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "white"="white",
              ...
              conditionalPanel(
                ...
                colorPicker(
                  ...
                  selected=c("#4DBBD5FF","#00A087FF","#3C5488FF","#F39B7FFF","#DC0000FF"),
                  plainColor = T,
                  multiple = TRUE
                ),
                #colorPicker(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        CrossErrortextvalue1<<-grnames1
        CrossErrorclassname<<-grnames
        CrossError_leibie_num<<-grnum1
        CrossError_fenzu_num<<-grnum2
        CrossErrordata<<-peaksdataout()
        if(ncol(CrossErrordata)==1){
          ggplot() + 
          ...
          library(ggplot2)
          library(ggsci)
          mean_A<-mean_B<-sd_A<-sd_B<-vector()
          for(i in 1:nrow(CrossErrordata)){
            numi1<-as.numeric(CrossErrordata[i,1:CrossError_fenzu_num[1]])
            mean_A[i]<-mean(numi1)
            sd_A[i]<-sd(numi1)
            numi2<-as.numeric(CrossErrordata[i,(CrossError_fenzu_num[1]+1):CrossError_fenzu_num[2]])
            mean_B[i]<-mean(numi2)
            sd_B[i]<-sd(numi2)
          }
          groupdf<-data.frame(Names=rownames(CrossErrordata),mean_A=mean_A,mean_B=mean_B,sd_A=sd_A,sd_B=sd_B)
          groupdf
          ggplot(groupdf,aes(x=mean_A,y=mean_B,color=Names))+
            geom_point(size=input$labelsizex)+
            geom_errorbar(aes(ymin=mean_B-sd_B, ymax=mean_B+sd_B), width=.2,size=1)+
            geom_errorbarh(aes(xmin=mean_A-sd_A, xmax=mean_A+sd_A),height=.2,size=1)+
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please change the error bar line width lower. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
ggplot(groupdf,aes(x=mean_A,y=mean_B,color=Names))+
   geom_point(size=5)+
   geom_errorbar(aes(ymin=mean_B-sd_B, ymax=mean_B+sd_B),width=0.2,size=0.5)+
   geom_errorbarh(aes(xmin=mean_A-sd_A, xmax=mean_A+sd_A),height=0.2,size=0.5)+
   scale_color_manual(values=pal_npg("nrc")(nrow(CrossErrordata)))+
