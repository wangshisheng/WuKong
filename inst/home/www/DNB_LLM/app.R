...
###########
ui <- dashboardPage(
  title = "Dynamical Network Biomarkers (DNB) with LLM Assistance",
  dashboardHeader(title = "Dynamical Network Biomarkers (DNB) with LLM Assistance", titleWidth = 490),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==1",
                textInput("grnums",h5("3.1. Time/Group and replicate number:"),value = ""),
                bsTooltip("grnums",'Type in the time/group number and replicate number here. Please note, the time/group number and replicate number are linked with ";", and the replicate number of each group is linked with "-". For example, if you have two groups, each group has three replicates, then you should type in "2;3-3" here. Similarly, if you have 3 groups with 5 replicates in every groups, you should type in "3;5-5-5".',
                          placement = "right",options = list(container = "body")),
                textInput("grnames",h5("3.2. Time/Group names:"),value = ""),
                bsTooltip("grnames",'Type in the time/group names of your samples. Please note, the time/group names are linked with ";". For example, there are two groups, you can type in "Control;Experiment".',
                          placement = "right",options = list(container = "body")),
                colorPicker(
                  inputId = "pointcolx1",
                  label = h5("3.3. Select a color for the line:"),
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "white"="white",
                    "grey"="grey",
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnums",h5("3.1. Time/Group and replicate number:"),value = "5;10-10-14-14-26"),
                textInput("examgrnames",h5("3.2. Time/Group names:"),value = "T1;T2;T3;T4;T5"),
                colorPicker(
                  inputId = "pointcolx2",
                  label = h5("3.3. Select a color for the line:"),
                  choices = list(
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        dnbtextvalue1<<-grnames1
        dnbclassname<<-grnames
        dnb_leibie_num<<-grnum1
        dnb_fenzu_num<<-grnum2
        dnbdata<<-peaksdataout()
        if(ncol(dnbdata)==1){
          datareadx<-data.frame(Description="No Results here. Please upload your dataset or use the example data.")
        }else{
          library(DNB)
          # New DNB object
          dnb <- new_DNB(data = dnbdata,time = as.factor(as.numeric(as.factor(dnbclassname))))
          # Calulate correlation and coefficient of variation
          dnb <- cal_cor(dnb)
          dnb <- cal_cv(dnb)
          # Search DNB
          dnb <- search_candidates(dnb, min_size = 20, max_size = 100)
          #Compare and get the time point with highest score, and use that set of DNB genes to recalculate DNB attibutes for all time points
          dnb <- cal_final(dnb)
          #Get DNB genes
          ...
          #datareadx <- get_final(dnb)
          datareadx<-data.frame(Markers=dnb_genes)
          datareadx
        }
        datareadx
      })
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(dnbdata)==1){
          ...
          p_score <- DNB:::plot_DNB_attr(final, "score")+geom_line(color = pointcolx)
          #p_cor_in <- DNB:::plot_DNB_attr(final, "cor_in")+geom_line(color = pointcolx)
          #p_cor_out <- DNB:::plot_DNB_attr(final, "cor_out")+geom_line(color = pointcolx)
          #p_cv <- DNB:::plot_DNB_attr(final, "cv_in")+geom_line(color = pointcolx)
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_DNB",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Show me the whole analysis codes and only output the DNB genes as a data frame for me. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
#Get DNB genes
dnb_genes <- get_DNB_genes(dnb)
dnb_genesdf<-data.frame(markers=dnb_genes)
dnb_genesdf
#Get candidate DNBs as data frame
candidates <- get_candidates(dnb)
candidates
...
final <- get_final(dnb)
final
#Plot DNB score from data frame
p_score <- DNB:::plot_DNB_attr(final, "score")+geom_line(color = pointcolx)
p_score
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DNB.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DNB.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
