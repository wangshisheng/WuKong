...
###########
ui <- dashboardPage(
  title = "Wilcoxon Rank Sum and Signed Rank Tests with LLM Assistance",
  dashboardHeader(title = "Wilcoxon Rank Sum and Signed Rank Tests with LLM Assistance", titleWidth = 640),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
    ...
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        if(ncol(DEPdata)==1){
          ...
          P.Value<-apply(DEPdata, 1, function(x) wilcox.test(x[1:DEP_fenzu_num[1]], x[(DEP_fenzu_num[1]+1):ncol(DEPdata)],alternative="two.sided",mu=0,paired=FALSE,exact=NULL,correct=TRUE)$p.value)
          ...
          datareadx<-cbind(DEPdata,Fold.Change=FCx,p.Value=P.Value,p.adjust=p.adjust)
          datareadx
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_DEPWilcox.table",usertimenum,".csv",sep="")},
        content = function(file){
  ...
  observeEvent(input$send, {
    ...
    if(length(user_inputx1)>0){
      typetext1<-'
DEPdata<-log2(DEPdata)
P.Value<-apply(DEPdata, 1, function(x) wilcox.test(x[1:DEP_fenzu_num[1]], x[(DEP_fenzu_num[1]+1):ncol(DEPdata)],alternative="two.sided",mu=0,paired=FALSE,var.equal = FALSE)$p.value)
p.adjust<-p.adjust(P.Value,method = "BH")
FCx<-rowMeans(DEPdata[,c((DEP_fenzu_num[1]+1):ncol(DEPdata))],na.rm = T)-rowMeans(DEPdata[,c(1:DEP_fenzu_num[1])],na.rm = T)
wilcoxtabledf<-cbind(DEPdata,Fold.Change=FCx,p.Value=P.Value,p.adjust=p.adjust)
wilcoxtabledf
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DEPWilcox.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DEPWilcox.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
