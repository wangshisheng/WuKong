...
###########
ui <- dashboardPage(
  title = "One-way ANOVA test with LLM Assistance",
  dashboardHeader(title = "One-way ANOVA test with LLM Assistance", titleWidth = 460),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnums",h5("3.1. Group and replicate number:"),value = "3;4-4-4"),
                textInput("examgrnames",h5("3.2. Group names:"),value = "A;B;C"),
                #colorPicker(
                #  inputId = "pointcolx2",
                #  label = h5("3.3. Select colors for each CV interval:"),
                #  choices = list(
              ...
              div(
                ...
                checkboxInput(
                  inputId = "logifx",
                  label = span("3.3. Log2 (base-2 logarithm) transformation used?", style = "font-size: 1em;"),
                  value = TRUE
                )
              ),
    ...
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        if(ncol(DEPdata)==1){
          ...
          P.Value<-apply(DEPdata, 1, function(x){
            x1<-data.frame(name=factor(DEPclassname,levels=DEPtextvalue1),numval=as.numeric(x))
            x2<-summary(aov(numval~name,data=x1))
            round(x2[[1]][["Pr(>F)"]][1],7)
          })
          p.adjust<-p.adjust(P.Value,method = "BH")
          datareadx<-cbind(DEPdata,p.Value=P.Value,p.adjust=p.adjust)
          datareadx
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_DEPAnnova.table",usertimenum,".csv",sep="")},
        content = function(file){
...
p.adjust<-p.adjust(P.Value,method = "BH")
aovtabledf<-cbind(DEPdata,p.Value=P.Value,p.adjust=p.adjust)
aovtabledf
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DEPAnnova.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DEPAnnova.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
