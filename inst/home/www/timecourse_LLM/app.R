...
###########
ui <- dashboardPage(
  title = "Time course data analysis with LLM Assistance",
  dashboardHeader(title = "Time course data analysis with LLM Assistance", titleWidth = 490),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              conditionalPanel(
                condition = "input.loaddatatype==2",
                textInput("examgrnums",h5("3.1. Group and replicate number:"),value = "4;3-3-3-3"),
              ...
              numericInput("labelsizex",h5("3.3. Point size:"),3),
            ...
            fluidRow(
              ...
              column(
                ...
                div(style = "display: flex; align-items:left;padding:0px;margin-left: 50px;margin-top: 0px;margin-bottom: 0px;",
                    ...
                    numericInput("heightx2", NULL, 500)
                )
              ),
              column(
                5,
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    load("fruitfly2.Rdata")
    dataread<-fruitfly2
    colnames(dataread)<-paste0(rep(c("T1","T2","T3","T4"),each=3),"_0",1:3)
    dataread
  })
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        timecoursedata<<-peaksdataout()
        if(ncol(timecoursedata)==1){
          datareadx<-data.frame(Description="No Results here. Please upload your dataset or use the example data.")
        }else{
          library(timecourse)
          timecoursetextvalue1<<-LETTERS[1:grnum2[1]]
          timecourseclassname<<-rep(timecoursetextvalue1, grnum1)
          timecourse_leibie_num<<-grnum1
          timecourse_fenzu_num<<-grnum2
          timecourse_n<-timecourse_leibie_num
          timecourse_assay <- timecourseclassname
          timecourse_time.grp <- rep(c(1:timecourse_n), timecourse_fenzu_num)
          timecourse_size <- rep(timecourse_fenzu_num[1], nrow(timecoursedata))
          timecourse_out <- mb.long(timecoursedata, times=timecourse_n, reps=timecourse_size, 
                                    rep.grp=timecourse_assay, time.grp=timecourse_time.grp)
          timecoursedata$HotellingT2<-timecourse_out$HotellingT2
          datareadx<-timecoursedata[order(timecoursedata$HotellingT2,decreasing = T),]
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_timecourse.table",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(timecoursedata)==1){
          ...
          timecourse_out <- mb.long(timecoursedata, times=timecourse_n, reps=timecourse_size, 
                                    rep.grp=timecourse_assay, time.grp=timecourse_time.grp)
          timecoursedata$HotellingT2<-timecourse_out$HotellingT2
          timecoursedata1<-timecoursedata[order(timecoursedata$HotellingT2,decreasing = T),]
          timecoursedata1$Groups<-"Not Highlighted"
          timecoursedata1$Groups[1:10]<-"Highlighted"
          timecoursedata1$Text<-""
          timecoursedata1$Seqs<-1:nrow(timecoursedata1)
          ggplot(data = timecoursedata1,aes(x=Seqs,y=HotellingT2,col=Groups))+
            geom_point(aes(size=Groups))+
            scale_color_manual(values = pointcolx)+
            scale_size_manual(values = c(isolate(input$labelsizex),1))+
            theme_classic()+
            xlab("Index")+ylab("HotellingT2")
        }
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_timecourse.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Show the distribution plot of the HotellingT2 values using ordered points. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
timecourse_time.grp <- rep(c(1:timecourse_n), timecourse_fenzu_num)
timecourse_size <- rep(timecourse_fenzu_num[1], nrow(timecoursedata))
timecourse_out <- mb.long(timecoursedata, times=timecourse_n, reps=timecourse_size, rep.grp=timecourse_assay, time.grp=timecourse_time.grp)
timecoursedata$HotellingT2<-timecourse_out$HotellingT2
timecoursedata1<-timecoursedata[order(timecoursedata$HotellingT2,decreasing = T),]
timecoursedata1$Groups<-"Not Highlighted"
timecoursedata1$Groups[1:10]<-"Highlighted"
...
ggplot(data = timecoursedata1,aes(x=Seqs,y=HotellingT2,col=Groups))+
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_timecourse.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_timecourse.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
