...
###########
ui <- dashboardPage(
  title = "Limma with LLM Assistance",
  dashboardHeader(title = "Limma with LLM Assistance", titleWidth = 280),
  dashboardSidebar(
    sidebarMenu(
...
server <- function(input, output, session) {
    ...
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        ...
        if(ncol(DEPdata)==1){
          ...
          datareadx<-cbind(DEPdata,Fold.Change=FCx,p.Value=wtEbFittable$P.Value,p.adjust=p.adjust)
          datareadx
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_DEPLimma.table",usertimenum,".csv",sep="")},
        content = function(file){
  ...
  observeEvent(input$send, {
    ...
    if(length(user_inputx1)>0){#DEPdata<-log2(DEPdata)
      typetext1<-'
library(limma)
log2if<-TRUE
if(log2if){
...
wtEbFittabledf<-cbind(DEPdata,Fold.Change=FCx,p.Value=wtEbFittable$P.Value,p.adjust=p.adjust)
wtEbFittabledf
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_DEPLimma.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_DEPLimma.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
