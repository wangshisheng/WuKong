...
###########
ui <- dashboardPage(
  title = "KEGG enrichment analysis with LLM Assistance",
  dashboardHeader(title = "KEGG enrichment analysis with LLM Assistance", titleWidth = 480),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              solidHeader = TRUE,
              collapsible = FALSE,
              textInput("keggspecies",h5("3.1. KEGG organism:"),value = "hsa"),
              colorPicker(
                inputId = "pointcolx",
              ...
              numericInput("idnumx",h5("3.3. Total number of Pathways displayed in the Barplot:"),15),
      ...
      tabItem(
        ...
        column(
          ...
          box(
            ...
            collapsible = FALSE,
            fluidRow(
              column(2, downloadButton("keggenrichllmplotdl", "Download")),
              column(
                5,
...
server <- function(input, output, session) {
  ...
  goenrichdefaulttableout<-reactive({
    ...
    if(length(grep("^No data",pronamevec))>0){
      ...
      idselect<<-input$keggspecies
      yy<<-enrichKEGG(gene=pronamevec,organism=idselect,keyType='uniprot',pvalueCutoff = 1,
                      qvalueCutoff=1,minGSSize=1,maxGSSize=500)
      yydf<-yy@result
      datareadx<-yydf[order(yydf$Count,decreasing = T),]
    ...
    input$mcsbtn_Barplot,{
      ...
      output$CVtableout<-renderDataTable({
        ...
        if(length(grep("^No data",allRes.df[[1]][1]))>0){
          ...
          allRes.df[[3]] <- paste0("<a href='https://www.genome.jp/dbget-bin/www_bget?pathway:",allRes.df[[3]],"' target='_blank'>",allRes.df[[3]],"</a>")
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_KEGGenrich.table",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(length(grep("^No data",allRes.df1[[1]][1]))>0){
          ...
          ggplot(allRes.df1.new, aes(x = reorder(Description,Count), y = Count,fill=p.adjust))+geom_bar(stat = "identity")+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_KEGGenrich.plot",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked1, {
    ...
    updateTextInput(session, "user_input", value = "Text: Now, I am studying lung cancer and care about inflammation, please provide me 3 most relative KEGG IDs, descriptions and relative protein uniprot ids based on provided contents.")
  })
  ...
  observeEvent(input$send, {
    ...
    if(length(grep("^Text:?",user_inputx,ignore.case = T))>0){
      ...
      allRes.df<<-goenrichdefaulttableout()
      allRes.dfx<-allRes.df[allRes.df$p.adjust<=0.2,]
      #allRes.dfx1<-paste0(allRes.dfx$GOIDs,": ",allRes.dfx$Description,", ",
      #                    allRes.dfx$Ontology,", ",allRes.dfx$IDs,collapse = "; ")
      allRes.dfx1<-paste0(allRes.dfx$ID,": ",allRes.dfx$subcategory,", ",
                          allRes.dfx$Description,", ",allRes.dfx$geneID,collapse = "; ")
      user_message<-paste0("Please review and learn the following KEGG data entries. Each entry contains the following fields: KEGG id, KEGG category, KEGG description, protein uniprot ids. Every KEGG content is pasted with a semicolon: ",allRes.dfx1,". ",input$user_input)
    ...
    new_chat <- lapply(chat_history(), function(x) {
      if (x$role == "user") {
        paste0(
          ...
          convert_to_html(gsub("Please review and learn the following KEGG data entries. Each entry contains the following fields: KEGG id, KEGG category, KEGG description, protein uniprot ids. Every KEGG content is pasted with a semicolon: .* Text: ","",
    ...
    output$keggenrichllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_KEGGenrich.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_KEGGenrich.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
