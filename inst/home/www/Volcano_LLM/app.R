...
###########
ui <- dashboardPage(
  title = "Volcano plot with LLM Assistance",
  dashboardHeader(title = "Volcano plot with LLM Assistance", titleWidth = 380),
  dashboardSidebar(
    sidebarMenu(
        ...
        fluidRow(
          column(
            ...
            box(
              ...
              solidHeader = TRUE,
              collapsible = FALSE,
              numericInput("pvaluex",h5("3.1. Threshold for P value:"),0.05),
              numericInput("fcvaluex",h5("3.2. Threshold for Fold Change:"),1.5),
              colorPicker(
                inputId = "colorx1",
                label = h5("3.3. Select colors for each group:"),
                choices = list(
                  "npg"=pal_npg("nrc")(10),
                  "Self-prepared colors"=c("blue","grey80","white","red"),
                  "aaas"=pal_aaas("default")(10),
                  "nejm"=pal_nejm("default")(8),
                  "pal"=scales::brewer_pal(palette = "Paired")(8)
                ), 
                selected=c("#3C5488FF","grey80","#EE0000FF"),
                plainColor = T,
                multiple = TRUE
              ),
...
server <- function(input, output, session) {
  ...
  examplepeakdatas<-reactive({
    load("volcanoshowdata.Rdata")
    volcanorawdata[[2]]<-log2(volcanorawdata[[2]])
    dataread<-volcanorawdata
    dataread
  })
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      CVtableoutx<-reactive({
        Volcanodata<<-peaksdataout()
        if(ncol(Volcanodata)==1){
          datareadx<-data.frame(Description="No Results here. Please upload your dataset or use the example data.")
        }else{
          datareadx<-Volcanodata
          datareadx[[1]][datareadx[[1]]==0]<-0.000001
          datareadx$Threshold<-c("NoChange")
          datareadx$Threshold[datareadx[[2]]>=log2(fcvaluex) & datareadx[[1]] <= pvaluex]<-c("UP")
          datareadx$Threshold[datareadx[[2]]<=log2(1/fcvaluex) & datareadx[[1]] <= pvaluex]<-c("DOWN")
          datareadx
        }
        datareadx
      })
      ...
      output$CVtableoutdl<-downloadHandler(
        filename = function(){paste("Default_Volcanotable",usertimenum,".csv",sep="")},
        content = function(file){
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        if(ncol(Volcanodata)==1){
          ...
          pvaluex<<-isolate(input$pvaluex)
          fcvaluex<<-isolate(input$fcvaluex)
          colorx<<-isolate(input$colorx1)
          library(ggplot2)
          volcanorawdata<-Volcanodata
          volcanorawdata[[1]][volcanorawdata[[1]]==0]<-0.000001
          volcanorawdata$Threshold<-c("NoChange")
          volcanorawdata$Threshold[volcanorawdata[[2]]>=log2(fcvaluex) & volcanorawdata[[1]] <= pvaluex]<-c("UP")
          volcanorawdata$Threshold[volcanorawdata[[2]]<=log2(1/fcvaluex) & volcanorawdata[[1]] <= pvaluex]<-c("DOWN")
          ggplot(data=volcanorawdata,aes(x=volcanorawdata[[2]],y=-log10(volcanorawdata[[1]]),
                                         colour=Threshold,size=Threshold))+
            theme_bw()+
            theme(panel.grid.minor = element_blank() ,panel.grid.major = element_blank())+
            geom_hline(yintercept =-log10(0.05),col="grey60",lty=2,lwd=1)+
            geom_vline(xintercept =log2(fcvaluex),col="grey60",lty=2,lwd=1)+
            geom_vline(xintercept =log2(1/fcvaluex),col="grey60",lty=2,lwd=1)+
            geom_point(alpha=0.9)+theme(legend.position ="right")+
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_Volcano",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "Please display the genes of interest listed below, Refer to inner codes:\n1635\n1712\n1751\n2228\n997")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
library(ggrepel)
volcanorawdata<-Volcanodata
colnames(volcanorawdata)<-c("P.Value","Fold.Change")
volcanorawdata$Gene<-rownames(volcanorawdata)
volcanorawdata[[1]][volcanorawdata[[1]]==0]<-0.000001
volcanorawdata$Threshold<-c("NoChange")
volcanorawdata$Threshold[volcanorawdata[[2]]>=log2(1.5) & volcanorawdata[[1]] <= 0.05]<-c("UP")
volcanorawdata$Threshold[volcanorawdata[[2]]<=log2(2/3) & volcanorawdata[[1]] <= 0.05]<-c("DOWN")
# Specify your interested gene codes
interested_genes <- c(1635, 1712, 1751, 2228, 997)
volcanorawdata$IsInterested <- ifelse(volcanorawdata$Gene %in% interested_genes, TRUE, FALSE)
ggplot(data=volcanorawdata,aes(x=Fold.Change,y=-log10(P.Value),
      colour=Threshold,size=Threshold))+
      theme_bw()+
      theme(panel.grid.minor = element_blank() ,panel.grid.major = element_blank())+
      geom_hline(yintercept =-log10(0.05),col="grey60",lty=2,lwd=1)+
      geom_vline(xintercept =log2(1.5),col="grey60",lty=2,lwd=1)+
      geom_vline(xintercept =log2(2/3),col="grey60",lty=2,lwd=1)+
      geom_point(alpha=0.9)+theme(legend.position ="right")+
      geom_text_repel(data = subset(volcanorawdata, IsInterested), aes(label = Gene),color = "black", size = 3.5) +
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_Volcano.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_Volcano.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
