...
###########
ui <- dashboardPage(
  title = "Hierarchical Clustering Analysis with LLM Assistance",
  dashboardHeader(title = "Hierarchical Clustering Analysis with LLM Assistance", titleWidth = 520),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                colorPicker(
                  inputId = "colorx1",
                  label = h5("3.4. Select three colors for heatmap:"),
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "white"="white",
                    "aaas"=pal_aaas("default")(10),
              ...
              conditionalPanel(
                ...
                colorPicker(
                  ...
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "Self‑provided colors"=c("blue","white","red"),
                    "aaas"=pal_aaas("default")(10),
                    "nejm"=pal_nejm("default")(8),
                    "pal"=scales::brewer_pal(palette = "Paired")(8)
                  ), 
                ...
                colorPicker(
                  ...
                  selected=c("#3C5488FF","white","#EE0000FF"),
                  plainColor = T,
                  multiple = TRUE
                ),
              ...
              numericInput("heightx",h5("3.6. Figure Height:"),800),
              numericInput("widthx",h5("3.7. Figure Width:"),700),
              hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        hcatextvalue1<<-grnames1
        hcaclassname<<-grnames
        hca_leibie_num<<-grnum1
        hca_fenzu_num<<-grnum2
        hcadata<<-peaksdataout()
        if(ncol(hcadata)==1){
          ggplot() + 
          ...
          annotation_col <- data.frame(
            Groups = factor(hcaclassname)
          )
          rownames(annotation_col) <- colnames(hcadata)
          names(pointcolx)<-hcatextvalue1
          ann_colors = list(
            Groups=pointcolx
          )
          pheatmap(hcadata, scale="row",fontsize_row = 10,fontsize_col = 10,show_rownames = F,
                   show_colnames = T,cluster_rows=T,cluster_cols=F,display_numbers=F,
                   annotation_col=annotation_col,border_color = NA,
                   color = colorRampPalette(colorx)(100),annotation_colors = ann_colors,
      ...
      output$DivergingBardefaultplotdl<-downloadHandler(
        filename = function(){paste("Default_HCA",usertimenum,".pdf",sep="")},
        content = function(file){
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "The vector of colors used in this heatmap are changed to c('blue','black','darkred'). Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
  ...
  observeEvent(input$send, {
    ...
    if(length(user_inputx1)>0){
      typetext1<-'
library(pheatmap)
annotation_col <- data.frame(Groups = factor(hcaclassname))
rownames(annotation_col) <- colnames(hcadata)
groupcolor<-c("darkblue","red")
names(groupcolor)<-hcatextvalue1
ann_colors <- list(Groups=groupcolor)
pheatmap(hcadata, scale="row",fontsize_row = 10,fontsize_col = 10,show_rownames = F,
          show_colnames = T,cluster_rows=T,cluster_cols=F,display_numbers=F,
          annotation_col=annotation_col,border_color = NA,
          color = colorRampPalette(c("blue","white","red"))(100),annotation_colors = ann_colors,
          clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",
          clustering_method = "complete")
      '
    ...
    output$code_output <- renderUI({
      ...
      #  HTML("")
      #}
      if(class(code_resultx)=="pheatmap"|is.ggplot(code_result())){
    ...
    output$plot_result <- renderPlot({
      if (!is.null(code_result()) && (class(code_resultx)=="pheatmap" | is.ggplot(code_result()))) {
        code_result()
      }
    })
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(class(code_resultx)=="pheatmap"|is.ggplot(code_result())){
          paste("Adjusted.by.LLM_HCA.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_HCA.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(class(code_resultx)=="pheatmap"|is.ggplot(code_result())){
