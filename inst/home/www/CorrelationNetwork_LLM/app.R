...
###########
ui <- dashboardPage(
  title = "Correlation Network plot with LLM Assistance",
  dashboardHeader(title = "Correlation Network plot with LLM Assistance", titleWidth = 500),
  dashboardSidebar(
    sidebarMenu(
          ...
          column(
            ...
            box(
              ...
              conditionalPanel(
                ...
                colorPicker(
                  inputId = "pointcolx1",
                  label = h5("3.3. Select colors for nodes:"),
                  choices = list(
                    "npg"=pal_npg("nrc")(10),
                    "white"="white",
                ...
                selectInput("cormethodx1",h5("3.5. Correlation method:"),choices = c("pearson","spearman","kendall"))
              ),
              conditionalPanel(
                condition = "input.loaddatatype==2",
              ...
              numericInput("labelsizex",h5("3.6. Label size:"),5),
              numericInput("heightx",h5("3.7. Figure Height:"),800),
              numericInput("widthx",h5("3.8. Figure Width:"),750),
              hr(),
...
server <- function(input, output, session) {
  ...
  observeEvent(
    input$mcsbtn_Barplot,{
      ...
      output$ClusterCorNetplot<-renderPlot({
        ...
        cornettextvalue1<<-grnames1
        cornetclassname<<-grnames
        cornet_leibie_num<<-grnum1
        cornet_fenzu_num<<-grnum2
        cornetdata<<-peaksdataout()
        if(ncol(cornetdata)==1){
          ggplot() + 
          ...
          library(igraph)
          library(ggraph)
          corrnet.test<-corr.test(cornetdata, method = cormethodx,adjust="BH")
          corrnet.matrix <- corrnet.test$r
          #corrnet.pmatrix <- corrnet.test$p
          #nett = which(abs(corrnet.matrix) > 0.8 & corrnet.pmatrix<0.05 & lower.tri(corrnet.matrix),arr.ind=TRUE)
          corrnet.matrix[!lower.tri(corrnet.matrix)]<-NA
          corrnet.matrix1<-melt(corrnet.matrix)
          corrnet.matrix2<-corrnet.matrix1[!is.na(corrnet.matrix1$value),]
          corrnet.nodes<-data.frame(Names=colnames(cornetdata),Groups=cornetclassname)
          nett.graph<-graph.data.frame(corrnet.matrix2,directed=F,vertices = corrnet.nodes)
          ggraph(nett.graph, layout="circle")+
            geom_edge_diagonal(aes(edge_width=value,color=value))+
            scale_edge_color_continuous(low = colorx[1],high = colorx[2])+
            geom_node_point(aes(color=Groups),size=10)+
            scale_color_manual(values = pointcolx)+
            geom_node_text(aes(label = name),colour="black", size = input$labelsizex, repel = TRUE)+
            theme_graph()
        }
  ...
  observeEvent(input$example_clicked, {
    ...
    updateTextInput(session, "user_input", value = "I want all point labels are bigger and two group point colors are changed to yellow and darkred. Refer to inner codes.")# and point shapes are circle
  })
  heightx2<-reactive({
    input$heightx2
  })
...
library(igraph)
library(ggraph)
corrnet.test<-corr.test(cornetdata, method = "pearson",adjust="BH")
corrnet.matrix <- corrnet.test$r
corrnet.matrix[!lower.tri(corrnet.matrix)]<-NA
corrnet.matrix1<-melt(corrnet.matrix)
corrnet.matrix2<-corrnet.matrix1[!is.na(corrnet.matrix1$value),]
...
ggraph(nett.graph, layout="circle")+
      ...
      geom_node_point(aes(color=Groups),size=10)+
      scale_color_manual(values = c("yellow","darkred"))+
      geom_node_text(aes(label = name),colour="black", size = 8, repel = TRUE)+
      theme_graph()
      '
    ...
    output$DivergingBarllmplotdl<-downloadHandler(
      filename = function(){
        if(is.ggplot(code_result())){
          paste("Adjusted.by.LLM_CorNet.plot",usertimenum,".pdf",sep="")
        }else{
          paste("Adjusted.by.LLM_CorNet.table",usertimenum,".csv",sep="")
        }
      },
      content = function(file){
        if(is.ggplot(code_result())){
